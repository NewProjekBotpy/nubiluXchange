...
import { MobileBottomNav, SellerSalesDashboard } from "@/components/admin";
    ...
    <div className="min-h-screen bg-nxe-dark pb-20">
      <div className="sticky top-0 z-40 bg-nxe-dark/95 backdrop-blur-sm border-b border-nxe-border">
        <div className="container mx-auto px-2 sm:px-3 py-1.5 sm:py-2 flex items-center gap-1.5 sm:gap-2">
          ...
          <div>
            <h1 className="text-sm sm:text-lg font-bold text-white">Sales Dashboard</h1>
            <p className="text-[10px] sm:text-xs text-gray-400">Sales analytics</p>
          </div>
        </div>
      </div>

      <div className="container mx-auto px-2 sm:px-3 py-2 sm:py-3">
        <SellerSalesDashboard hasAdminAccess={hasAdminAccess} />
      </div>

      <MobileBottomNav activeTab="sales" onTabChange={() => {}} />
    </div>
  );
}
